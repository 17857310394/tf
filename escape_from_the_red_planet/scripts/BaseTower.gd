...
# 核心属性
var tower_data: TowerData  # 防御塔数据
...
# 信号
signal on_target_acquired(target)  # 获得目标时触发
signal on_target_lost()  # 失去目标时触发
signal on_attack_started()  # 开始攻击时触发
signal on_attack_completed()  # 攻击完成时触发
signal on_upgraded(new_level)  # 升级时触发
signal on_sold()  # 出售时触发
signal on_destroyed()  # 销毁时触发
